...
x = keras.layers.Conv2D(16,  kernel_size=(filter_size, filter_size), activation=activation_function)(model_input)
x = keras.layers.AveragePooling2D(pool_size=(pooling_size, pooling_size))(x)
x = keras.layers.Conv2D(32,  kernel_size=(filter_size, filter_size), activation=activation_function)(x)
x = keras.layers.AveragePooling2D(pool_size=(pooling_size, pooling_size))(x)
x = keras.layers.Conv2D(64,  kernel_size=(filter_size, filter_size), activation=activation_function)(x)
x = keras.layers.AveragePooling2D(pool_size=(pooling_size, pooling_size))(x)
x = keras.layers.Flatten()(x)
x = keras.layers.Dense(256, activation=activation_function)(x)
x = keras.layers.Dense(128, activation=activation_function)(x)
x = keras.layers.Dense(64, activation=activation_function)(x)
model_output = keras.layers.Dense(nParams, activation="linear")(x)
...
model.save(folder_path + "OutputData/SavedModels/CNN_model.keras")
#drive.flush_and_unmount()

####################################### MODEL EVALUATION #####################################################

score = model.evaluate(x_test, y_test, verbose=1)  # loss is score[0], acc is score[1]
np.savetxt(folder_path +'OutputData/score' + '.txt', score, delimiter=',',fmt='%f')
np.savetxt(folder_path +'OutputData/MinMax' + '.txt', np.c_[minlist,maxlist], delimiter=' ',fmt='%f')
...
#np.savetxt( folder_path + 'OutputData/history' + '.txt', [x1,x2], delimiter=',',fmt='%f')
...
#np.savetxt(folder_path + 'OutputData/actual.txt', actual, delimiter=',',fmt='%f')
#np.savetxt(folder_path + 'OutputData/pred.txt', pred, delimiter=',',fmt='%f')
...
for i in range(0,nParams):
    ...
    #plt.savefig(folder_path + 'OutputData/' + str(string_list[i]) + '.jpg')
